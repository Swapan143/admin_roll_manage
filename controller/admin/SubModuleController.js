...
const userList = asyncHandler(async(req, res) => 
{
    try
    {
        ...
        res.render('admin/layouts/master/master',{page_title:"Admin |Sub Module List", active_menu:"ModuleController",page_name:'submodule/index',moduleId});
    }
    catch(err)
    {
...
const getDataAjax = asyncHandler(async(req, res) => 
{
    try
    {
        const { searchFdate, searchTdate,module_id, draw, start, length } = req.query;

        // Set up a query object to filter users based on date range
        let query = { parent_id: module_id};
        ...
        const data = filteredModule.map((module, index) => {
            ...
            return {
                ...
                action: `<a href="/admin/module/edit/${module._id}" class="btn btn-primary">Edit</a>`
            };
        });
...
const userAdd = asyncHandler(async(req, res) => 
{
    ...
    {
        let moduleId = req.params.id;
        res.render('admin/layouts/master/master',{page_title:"Admin | Sub Module Add", active_menu:"ModuleController",page_name:'submodule/add',moduleId});
    }
    catch(err)
    {
        req.flash('error','Something went wrong');
...
const userStore = asyncHandler(async(req, res) => 
{
    try 
    {
        ...
        const { module_name,module_url,module_icon,controller_name,module_id} = req.body;
        ...
        if (module) 
        {
            ...
            res.redirect(`/admin/submodule/add/${module_id}`); 
            return; 
        } 
        else 
        {
            ...
            res.redirect(`/admin/submodule/list/${module_id}`); 
            return;
